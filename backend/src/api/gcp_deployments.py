...
async def build_gcp_image(
    ...
    try:
        ...
        if not gcp_creds:
            raise HTTPException(status_code=400, detail="GCP credentials not found. Please connect your GCP account.")
...
async def plan_gcp_deployment(
    ...
    try:
        ...
        if not build_logs:
            raise HTTPException(status_code=400, detail="Image not built yet. Please build the image first.")
        ...
        if not image_uri:
            raise HTTPException(status_code=400, detail="Could not find image URI. Please rebuild the image.")
        ...
        async with sandbox:
            ...
            bucket_name = await ensure_gcs_state_bucket(credentials, gcp_project_id, service_name, sandbox)
...
"""
            ...
            plan_result = await sandbox.run_terraform("plan", tf_dir, var_file="terraform.tfvars", env_vars=terraform_env)
...
"""
            ...
            apply_result = await sandbox.run_terraform("apply", tf_dir, var_file="terraform.tfvars", auto_approve=True, env_vars=terraform_env)
...
async def destroy_gcp_deployment(
    ...
    try:
        ...
        async with sandbox:
            ...
            from src.services.deployment.gcs_state_manager import ensure_gcs_state_bucket, cleanup_terraform_state
            from src.api.gcp_auth import get_gcp_credentials
...
"""
            ...
            destroy_result = await sandbox.run_terraform("destroy", tf_dir, var_file="terraform.tfvars", auto_approve=True, env_vars=terraform_env)
