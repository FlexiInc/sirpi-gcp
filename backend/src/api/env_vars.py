...
async def get_project_env_vars(
    ...
    try:
        ...
        # Get env vars (values will be masked by service layer)
        env_vars = await get_env_vars_for_project(project_id)
        
        return [
            EnvVarResponse(
                key=var["key"],
                value=None if var["is_secret"] else var.get("value"),
...
async def get_env_vars_for_project(project_id: str) -> List[Dict]:
    """Get all env vars for a project from database."""
    try:
        with supabase.get_connection() as conn:
            with conn.cursor() as cur:
                cur.execute(
                    """
                    SELECT key, is_secret, description, created_at
    ...
    try:
        with supabase.get_connection() as conn:
            with conn.cursor() as cur:
                ...
                return {
                    row["key"]: decrypt_value(row["value_encrypted"])
                    for row in rows
                }
    except Exception as e:
