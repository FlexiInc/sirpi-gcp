...
class CloudRunTemplateGenerator:
    ...
    def generate(
        ...
        **kwargs,
        ...
        """Generate Cloud Run Terraform files. Backend added dynamically at deploy time."""
        app_name = (
            repo_full_name.split("/")[-1].lower() if repo_full_name else f"app-{project_id[:8]}"
        )
...
resource "google_cloud_run_v2_service" "main" {{
  ...
  template {{
    containers {{
      # Image from shared registry: {registry_repo}
      image = "${{var.region}}-docker.pkg.dev/${{var.project_id}}/{registry_repo}/{app_name}:latest"
      ...
      # Health checks disabled by default for initial deployment
      # Uncomment after first successful deployment to enable health monitoring
      # startup_probe {{
      #   http_get {{
      #     path = "{analysis_result.health_check_path or "/health"}"
      #     port = {port}
      #   }}
...
'''
        ...
        # variables.tf
        files["variables.tf"] = f'''variable "project_id" {{
